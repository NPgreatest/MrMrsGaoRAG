...
async def search_faiss(query, top_k=5, min_threshold=0.85, context_window=2):
    """Search FAISS asynchronously and retrieve matching results with context handling."""
    ...
    def blocking_faiss_search():
        """Runs FAISS search in a blocking manner, to be executed in a separate thread."""
        ...
        for i, idx in enumerate(indices[0]):
            ...
            if similarity >= min_threshold:
                neighbors = [metadata_list.get(int(idx + offset), {}) for offset in
                             range(-context_window, context_window + 1) if (idx + offset) in metadata_list]
    ...
    # Execute blocking FAISS search in a separate thread
    return await asyncio.to_thread(blocking_faiss_search)
